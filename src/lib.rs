...
use crate::Cardinality::{ExactlyOne, OneOrMore, ZeroOrMore, ZeroOrOne};
use crate::Identification::{Identifying, NonIdentifying};
...
mod tests {
    ...
    use crate::Cardinality::{OneOrMore, ZeroOrMore, ZeroOrOne};
    use crate::Identification::NonIdentifying;
    use crate::KeyConstraint::{FK, PK, UK};
    ...
    fn test_parse_relationship() {
        assert_eq!(relationship("|o to o|"), Ok(("", Relationship {
            left_cardinality: ZeroOrOne,
            identification: Identifying,
            right_cardinality: ZeroOrOne,
        })));

        assert_eq!(relationship("|o .. o|"), Ok(("", Relationship {
            left_cardinality: ZeroOrOne,
        ...
        assert_eq!(relationship("one or zero to zero or one"), Ok(("", Relationship {
            left_cardinality: ZeroOrOne,
            identification: Identifying,
            right_cardinality: ZeroOrOne,
        })));

        assert_eq!(relationship("zero or one optionally to one or zero"), Ok(("", Relationship {
            left_cardinality: ZeroOrOne,
            identification: NonIdentifying,
            right_cardinality: ZeroOrOne,
        })));
    ...
    fn test_parse_entity_relation() {
        assert_eq!(entity_relation("\"CAR\" }o--|| \"NAMED-DRIVER\" : \"foo\""), Ok(("", EntityRelation {
            left: Entity("CAR"),
        ...
        assert_eq!(entity_relation("PERSON 0+ to |{ NAMED-DRIVER : is"), Ok(("", EntityRelation {
            left: Entity("PERSON"),
            right: Entity("NAMED-DRIVER"),
            label: Label("is"),
            relationship: Relationship {
        ...
        assert_eq!(entity_relation(r#"CUSTOMER ||--o{ INVOICE : "places""#), Ok(("", EntityRelation {
    ...
    fn test_parse_attribute_spec() {
        assert_eq!(attribute_spec("date payment_date"), Ok(("", Attribute {
            typ: Type("date"),
            name: "payment_date",
            constraints: vec![],
            comment: None,
    ...
    fn test_parse_attributes_section() {
        assert_eq!(entity_attributes("CAR { string name }"), Ok(("", EntityAttributes {
            name: Entity("CAR"),
        ...
        assert_eq!(entity_attributes(r#"CAR
{
    ...
    int age
        ...
        assert_eq!(entity_attributes(r#""01 TRUCK"
{
    ...
    string name "User Name" string[] components
        ...
        assert_eq!(entity_attributes(r#""01 TRUCK" { int id UK "Primary Key" string name "User Name" string[] components } "#),
                   Ok(("", EntityAttributes {
                       name: Entity("01 TRUCK"),
                       attributes: vec![
                           Attribute {
    ...
    fn test_parse_diagram_section() {
        assert_eq!(diagram_section("CAR { string name }"), Ok(("", DiagramSection::EntityAttributes(EntityAttributes {
            name: Entity("CAR"),
            attributes: vec![Attribute {
                typ: Type("string"),
                name: "name",
